//17.6 Random file access with fseek functions; p 438
...
int main(void)
{
    ...
    if((fp = fopen("kamrandom.dat","wb")) == NULL)
    {
        fprintf(stderr, "\nError opening file...");
        exit(1);
    }
    //write the array to the file and then close it
    ...
    if((fp = fopen("kamrandom.dat","rb")) == NULL)
    {
        fprintf(stderr, "\nError opening file...");
        exit(1);
    }
    //ask user which element to read, input the element and display it, quitting
    //  when -1 is entered
    while(1)
    {
